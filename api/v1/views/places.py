...
def handle_places(city_id=None, place_id=None):
    """The handler for the places endpoint.
    """
    handlers = {
        'GET': get_places,
        'DELETE': remove_place,
...
def get_places(city_id=None, place_id=None):
    '''Retrieve the place with the given id or all places in
...
def remove_place(city_id=None, place_id=None):
    '''removing a place with the given id.
    '''
    if place_id:
        place = storage.get(Place, place_id)
        if place:
...
def update_place(city_id=None, place_id=None):
    '''Updateing the place with the given id.
    '''
...
def find_places():
    '''To finds places based on list of State, City, or Amenity ids.
    '''
    data = request.get_json()
    if type(data) is not dict:
        raise BadRequest(description='Not a JSON')
